...
export default function InvestorPage() {
  // Initialisation à l'intérieur pour garantir l'accès aux variables d'env
  ...
  const roi = investment > 0 ? (realRevenueUSD / investment) * 100 : 0;

  return (
    <div className="max-w-5xl mx-auto space-y-8 p-4 bg-black min-h-screen text-white">
      <h1 className="text-3xl font-black italic tracking-tighter">PERFORMANCE INVESTISSEUR</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#7c63f5] p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
             ...
             <h2 className="text-7xl font-black my-4 tracking-tighter">{roi.toFixed(2)}%</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#121214] border border-slate-800 p-6 rounded-[32px]">
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase">Objectif (Initial)</p>
      ...
      <div className="bg-[#121214] border border-slate-800 p-8 rounded-[40px]">
        <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
          <TrendingUp size={20} className="text-[#7c63f5]" /> Ajuster mon investissement initial
        </h3>
        <input 
        ...
        <p className="text-center text-slate-500 text-sm">
          Si j'investis <span className="text-white font-bold">{investment.toLocaleString()} $</span>, mon ROI actuel est de <span className="text-[#22c55e] font-bold">{roi.toFixed(1)}%</span>
        </p>
      </div>
    </div>
  );
